...
def iupred_comp(gene_list, hit_list):
    iu_q = float(gene_list[47])
    iu_t = [float(i[47]) for i in hit_list]
    if len(set(iu_t)) == 1:
        return 'Z'
    with np.errstate(divide='ignore'):
        p_value = stats.ttest_1samp(iu_t, iu_q)[1]
    if p_value <= 0.05:
        if iu_q > np.mean(iu_t):
            return 'A'
        else:
            return 'B'
    else:
...
for gene in JG3_genes:
    gene = gene[0]
    gene_data = conn.execute("""SELECT * FROM aa_analysis_iupred WHERE query_id={0}
                                AND target_id={0}""".format(gene)).fetchall()
    hit_data = conn.execute("""SELECT * FROM aa_analysis_iupred WHERE query_id={0}
                               AND target_id!={0}""".format(gene)).fetchall()
    for result in gene_data:
        ...
        gravy_comp(result, hit_data),'\t', iupred_comp(result, hit_data)